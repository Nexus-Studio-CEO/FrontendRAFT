...
// types/index.ts
var RAFTError = class extends Error {
...
__export(jwt_exports, {
  ...
  getTokenExpiryDate: () => getTokenExpiryDate,
...
function useQuery(raft, resourceType, options) {
...
function useOptimistic(raft, resourceType) {
  const [data, setData] = useState(null);
  const [pending, setPending] = useState(false);
  const create = useCallback(async (optimisticData, actualRequest) => {
    setPending(true);
...
function useStream(raft, channel) {
...
function useCache(raft, key) {
  ...
  const setCache = useCallback(async (value, options) => {
